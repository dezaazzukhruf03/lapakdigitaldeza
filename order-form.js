// AMBIL KODE PESANAN DARI URL (?kode=xxxxx)
const urlParams = new URLSearchParams(window.location.search);
const orderCode = urlParams.get("kode");
...
function showState(stateEl) {
    [stateLoading, stateNotFound, stateLocked, stateSuccess, orderForm].forEach(el => {
        el.style.display = "none";
    });
    stateEl.style.display = stateEl === orderForm ? "block" : "block";
}
...
async function loadOrder() {
    ...
    const { data, error } = await sb.rpc("get_order_for_form", { p_order_code: orderCode });
    ...
    // Isi bagian read-only
    document.getElementById("templateTitleDisplay").value = order.template_title || "-";
    document.getElementById("customerNameDisplay").value = order.customer_name || "-";

    // Tampilkan tombol folder Drive kalau Admin sudah menyiapkan link-nya
    if (order.drive_folder_link) {
        const driveSection = document.getElementById("driveSection");
        const driveLink = document.getElementById("driveFolderLink");
        driveLink.href = order.drive_folder_link;
...
function collectGiftAccounts() {
    ...
    rows.forEach(row => {
        ...
        // Hanya masukkan kalau minimal nama bank & nomor rekening diisi
...
orderForm.addEventListener("submit", async function (e) {
    ...
    const { error } = await sb.rpc("submit_order_details", payload);

    if (error) {
        console.error("Gagal submit:", error);
        showFormError("Gagal mengirim data. Coba lagi, atau hubungi Admin kalau masih gagal.");
        submitBtn.disabled = false;
        submitBtn.textContent = "Kirim Data Undangan";
        return;
    }
